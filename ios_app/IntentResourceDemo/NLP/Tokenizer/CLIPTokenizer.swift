...
class CLIPTokenizer {
    ...
    init?() {
        ...
        guard let mergesURL = Bundle.main.url(forResource: "clip-merges", withExtension: "txt"),
              let vocabURL = Bundle.main.url(forResource: "clip-vocab", withExtension: "json") else {
            return nil
        }

        guard let bpeMergesTxt = try? String(contentsOf: mergesURL),
              let json = try? Data(contentsOf: vocabURL),
              let vocab = try? JSONDecoder().decode([String: Int].self, from: json) else {
            return nil
        }
